...
const TrendBottom = ({forTrendBottom}) => {
  ...
  function elements(forTrendBottom, dateFormat) {
    ...
    for (let i = 0; i < articles.length; i++) {
      if (i % 2 === 0) {
        let article = <div className="col-md-4 col-sm-6" key={i + 1}>{articles[i]}{articles[i + 1]}</div>;
        result.push(article);
      }
    }
    return result;
  ...
  return (
    <MyContext.Consumer>
      {
        (dateFormat) => {
          const itemList = forTrendBottom ? elements(forTrendBottom, dateFormat) : forTrendBottom; 
          return (
            <div className="row">
              {itemList}
            </div>
